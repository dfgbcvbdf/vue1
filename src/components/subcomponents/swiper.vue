<template>
    <mt-swipe :auto="4000">
        ...
        <!-- 将来，谁使用此 轮播图组件，谁为我们传递 imgList -->
        <!-- imgList 应该是 父组件向子组件传值来设置 -->
            <mt-swipe-item v-for="item in imgList" :key="item.id" > <!-- 在组件中，使用v-for循环的话，一定要使用 key -->
                <img :src="item.img" :class="{full: isFull}"><!-- img 中正确使用src的方法-->
            </mt-swipe-item>
        </mt-swipe>
